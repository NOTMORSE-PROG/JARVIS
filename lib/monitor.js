...
async function handlePayload(payload) {
  ...
  if (payload.event_type === 'build_status') {
    const emoji = payload.status === 'success' ? '✅' : '❌';
    const message = `${emoji} *${payload.workflow}: ${payload.status}* on \`${payload.branch}\`\n<${payload.url}|View Workflow>`;
    await slack.sendNotification(message);
  } else if (payload.event_type === 'push') {
    const message = `🔨 *New Push* to \`${payload.branch}\` in <https://github.com/${payload.owner}/${payload.repo}|${payload.repo}>\n*${payload.message}* by ${payload.pusher}`;
    await slack.sendNotification(message);
  } else if (payload.event_type === 'create') {
    const message = `🌱 *New Branch Created* \`${payload.branch}\` in <https://github.com/${payload.owner}/${payload.repo}|${payload.repo}>\nby ${payload.sender}`;
    await slack.sendNotification(message);
  } else if (payload.event_type === 'pull_request') {
    ...
    let message = '';
    if (action === 'opened') {
      message = `🚨 *New Pull Request* in <https://github.com/${payload.owner}/${payload.repo}|${payload.repo}>\n*${prTitle}* by ${user}\n<${prUrl}|View PR>`;
    } else if (action === 'closed' && payload.merged) {
      message = `🟣 *Pull Request Merged* in <https://github.com/${payload.owner}/${payload.repo}|${payload.repo}>\n*${prTitle}* by ${user}\n<${prUrl}|View PR>`;
    } else if (action === 'closed' && !payload.merged) {
      message = `⚪ *Pull Request Closed* in <https://github.com/${payload.owner}/${payload.repo}|${payload.repo}>\n*${prTitle}* by ${user}\n<${prUrl}|View PR>`;
    }
